...
def main():
    ...
    for o, a in opts:
        if o in ("-h", "--help"):
            ...
            outputFormat = a
            # process user do not assign a format
            if outputFormat in ("-o", "--output"):
                print "please at least assign a format"
                usage()
                sys.exit(0)
            # process user uses other format that output_format does not have
            try:
                getattr(output_format, "output_%s" %outputFormat)
            except AttributeError:
                outputFormat = output_format.output_csv
            ...
            if outputFileName is None:
                print "you do not assign a file, use default data.xls"
...
def output_function(classInstance, data, outputFileName, outputFormat="csv"):
    output_function = getattr(classInstance, "output_%s" %outputFormat, classInstance.output_csv)
